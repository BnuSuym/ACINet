...
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        for i, (images, gts, depth) in enumerate(train_loader, start=1):
            ...
            s, s2 = model(images, depth)

            sal1_loss = structure_loss(s, gts)
            sal2_loss = structure_loss(s2, gts)
            ...
            if i % 100 == 0 or i == total_step or i == 1:
                print(
                    '{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR:{:.7f}||sal1_loss:{:4f}||sal2_loss:{:4f}'.
                    format(datetime.now(), epoch, opt.epoch, i, total_step,
                           optimizer.state_dict()['param_groups'][0]['lr'], sal1_loss.data, sal2_loss.data))
                logging.info(
                    '#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR:{:.7f},  sal1_loss:{:4f},  sal2_loss:{:4f}, mem_use:{:.0f}MB'.
                    format(epoch, opt.epoch, i, total_step, optimizer.state_dict()['param_groups'][0]['lr'],
                           sal1_loss.data, sal2_loss.data, memory_used))
        ...
        if (epoch) % 5 == 0:
            torch.save(model.state_dict(), save_path + 'ACINet_epoch_{}.pth'.format(epoch))
    except KeyboardInterrupt:
        print('Keyboard Interrupt: save model and exit.')
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(model.state_dict(), save_path + 'ACINet_epoch_{}.pth'.format(epoch + 1))
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + 'ACINet_epoch_best.pth')
